...
<div class="max-w-4xl mx-auto space-y-4 sm:space-y-6">
    <div class="bg-white rounded-xl border border-gray-100 p-4 sm:p-6">
        <div class="flex items-center space-x-3 sm:space-x-4">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Nuevo Servicio</h1>
                <p class="text-xs sm:text-sm text-gray-500 mt-1">Crea un servicio predefinido para facturación electrónica</p>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="code_reference" name="code_reference" value="{{ old('code_reference') }}"
                ...
                <div>
                    ...
                    <select id="unit_measure_id" name="unit_measure_id"
                        ...
                        @foreach($unitMeasures as $unit)
                            <option value="{{ $unit->factus_id }}" {{ old('unit_measure_id') == $unit->factus_id ? 'selected' : '' }}>
                                {{ $unit->name }} ({{ $unit->code }})
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="block w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">{{ old('description') }}</textarea>
            </div>
                ...
                <div>
                    ...
                    <select id="standard_code_id" name="standard_code_id"
                        ...
                        @foreach($standardCodes as $code)
                            <option value="{{ $code->id }}" {{ old('standard_code_id') == $code->id ? 'selected' : '' }}>
                                {{ $code->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="tribute_id" name="tribute_id"
                        ...
                        @foreach($tributes as $tribute)
                            <option value="{{ $tribute->id }}" {{ old('tribute_id') == $tribute->id ? 'selected' : '' }}>
