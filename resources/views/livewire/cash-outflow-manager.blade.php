...
<div class="space-y-6" wire:poll.5s x-data="{ confirmingDelete: false, deleteId: null }">
            ...
            <div class="flex items-center space-x-3 sm:space-x-4">
                ...
                <div>
                    ...
                    <p class="text-xs sm:text-sm text-gray-500 mt-1">
                        Registre gastos pagados desde la caja (ventas + base). Esto afecta el cuadre del turno cuando hay turno activo.
                    </p>
                </div>
            </div>
    ...
    @if (session()->has('success'))
        <div class="bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-xl flex items-center shadow-sm animate-in fade-in slide-in-from-top-2">
    ...
    @if (session()->has('error'))
        <div class="bg-rose-50 border border-rose-200 text-rose-700 px-4 py-3 rounded-xl flex items-center shadow-sm animate-in fade-in slide-in-from-top-2">
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 shadow-sm">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <!-- Fecha -->
            <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-widest mb-2 ml-1">Filtrar por Fecha</label>
                <div class="relative">
            ...
            <!-- Buscar por Motivo -->
            <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-widest mb-2 ml-1">Buscar por Motivo</label>
                <div class="relative">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-gray-50/50">
                        <tr>
                            <th class="px-4 py-3.5 text-left text-[10px] font-black text-gray-500 uppercase tracking-widest">Fecha/Hora</th>
                            <th class="px-4 py-3.5 text-left text-[10px] font-black text-gray-500 uppercase tracking-widest">Recepcionista</th>
                            <th class="px-4 py-3.5 text-left text-[10px] font-black text-gray-500 uppercase tracking-widest">Motivo</th>
                            <th class="px-4 py-3.5 text-right text-[10px] font-black text-gray-500 uppercase tracking-widest">Monto</th>
                            @if(($isAdmin ?? false) || (($activeShiftId ?? null) && ($activeShiftStatus ?? '') === 'activo'))
                            <th class="px-4 py-3.5 text-right text-[10px] font-black text-gray-500 uppercase tracking-widest">Acciones</th>
                            @endif
                        </tr>
                    </thead>
                        ...
                        @foreach($outflows as $outflow)
                            <tr class="hover:bg-gray-50/50 transition-colors duration-150 group">
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-900">{{ $outflow->date->format('d/m/Y') }}</div>
                                    <div class="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">{{ $outflow->date->format('H:i') }}</div>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center text-gray-500 mr-2 text-[10px] font-bold">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-right">
                                    <div class="text-sm font-black text-rose-600">-${{ number_format($outflow->amount, 0, ',', '.') }}</div>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap text-right">
                                    <button type="button" 
            ...
            <div class="px-4 py-4 border-t border-gray-100 bg-gray-50/30">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-xs text-gray-500">
                        Mostrando <span class="font-bold text-gray-900">{{ $outflows->firstItem() }}</span> a <span class="font-bold text-gray-900">{{ $outflows->lastItem() }}</span> de <span class="font-bold text-gray-900">{{ $outflows->total() }}</span> registros
                    </div>
                    <div class="bg-rose-50 px-4 py-2 rounded-xl border border-rose-100">
                        <span class="text-[10px] font-black text-rose-600 uppercase tracking-widest mr-2">Total Salidas:</span>
                        <span class="text-sm font-black text-rose-700">-${{ number_format($totalOutflows, 0, ',', '.') }}</span>
                    </div>
                </div>
                <div class="mt-4">
                    {{ $outflows->links() }}
        ...
        @else
            <div class="p-16 text-center">
                <div class="inline-flex items-center justify-center w-16 h-16 rounded-3xl bg-gray-50 mb-4 text-gray-300">
    ...
    <div class="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div class="relative transform overflow-hidden rounded-[2.5rem] bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg animate-in zoom-in-95 duration-200">
                <form wire:submit.prevent="saveOutflow">
                    <div class="bg-white px-6 pt-8 pb-6 sm:p-10">
                        <div class="flex items-center space-x-4 mb-8">
                            ...
                            <div>
                                <h3 class="text-xl font-black text-gray-900 uppercase tracking-widest">Nueva Salida</h3>
                        ...
                        <div class="space-y-6">
                            <!-- Monto -->
                            <div>
                                <label class="block text-xs font-bold text-gray-700 uppercase tracking-widest mb-2 ml-1">Monto de la Salida <span class="text-rose-500">*</span></label>
                                <div class="relative">
                                ...
                                @error('amount') <p class="mt-2 text-[10px] text-rose-600 font-bold uppercase ml-1 tracking-tighter">{{ $message }}</p> @enderror
                            </div>

                            <!-- Fecha -->
                            <div>
                                <label class="block text-xs font-bold text-gray-700 uppercase tracking-widest mb-2 ml-1">Fecha de la Salida <span class="text-rose-500">*</span></label>
                                <input type="date" wire:model="outflow_date" 
                                       max="{{ date('Y-m-d') }}"
                                ...
                                @error('outflow_date') <p class="mt-2 text-[10px] text-rose-600 font-bold uppercase ml-1 tracking-tighter">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                @error('reason') <p class="mt-2 text-[10px] text-rose-600 font-bold uppercase ml-1 tracking-tighter">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>

                    <div class="bg-gray-50 px-6 py-6 sm:px-10 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                        <button type="button" 
    ...
    <div x-show="confirmingDelete" 
         ...
         x-cloak>
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity" @click="confirmingDelete = false"></div>

            <div class="relative transform overflow-hidden rounded-[2.5rem] bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md animate-in zoom-in-95 duration-200">
                <div class="bg-white px-6 pt-8 pb-6 sm:p-10 text-center">
                    ...
                    <h3 class="text-xl font-black text-gray-900 uppercase tracking-widest mb-2">¿Eliminar Registro?</h3>
                    <p class="text-sm text-gray-500 font-medium">Esta acción eliminará permanentemente esta salida de dinero. ¿Estás seguro?</p>
                </div>
                <div class="bg-gray-50 px-6 py-6 flex flex-col sm:flex-row justify-center gap-3">
                    ...
                    <button type="button" 
                            wire:click="deleteOutflow(deleteId); confirmingDelete = false"
