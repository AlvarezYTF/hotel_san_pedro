    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 sm:p-6">
        <div class="flex items-center space-x-3 sm:space-x-4">
            <div class="p-2.5 sm:p-3 rounded-xl bg-violet-50 text-violet-600">
                ...
                <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Nueva Categoría</h1>
                <p class="text-xs sm:text-sm text-gray-500 mt-1">Organiza tus productos creando una nueva categoría con su color distintivo</p>
                </div>
            </div>
        </div>
        
    <form method="POST" action="{{ route('categories.store') }}" id="category-form" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            ...
            <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-4 sm:p-6 border border-gray-200">
                <div class="flex items-center space-x-3 sm:space-x-4">
                    <div class="h-12 w-12 sm:h-14 sm:w-14 rounded-xl flex items-center justify-center shadow-sm border border-gray-200 flex-shrink-0"
                        ...
                        <i class="fas fa-tag text-lg sm:text-xl" style="color: #3B82F6;"></i>
                        </div>
                    <div class="flex-1 min-w-0">
                        <div class="text-base sm:text-lg font-semibold text-gray-900 truncate" id="preview-name">Nombre de la categoría</div>
                        <div class="text-xs sm:text-sm text-gray-600 mt-0.5 line-clamp-2" id="preview-description">Descripción de la categoría</div>
                            <div class="mt-2">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold"
                                      id="preview-badge" 
                                      style="background-color: #3B82F620; color: #3B82F6;">
                                <i class="fas fa-circle mr-1.5 text-xs" style="color: #3B82F6;"></i>
                                    Categoría activa
                                </span>
                            </div>
                        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-4 border-t border-gray-100">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                   ...
                   class="inline-flex items-center justify-center px-4 sm:px-5 py-2.5 rounded-xl border-2 border-gray-200 bg-white text-gray-700 text-sm font-semibold hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                    ...
                    <span x-text="loading ? 'Procesando...' : 'Crear Categoría'">Crear Categoría</span>
                    </button>
                </div>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        const color = colorText.value || '#3B82F6';
    ...
    if (colorOptions.length > 0 && colorText.value === '#3B82F6') {
        colorOptions[0].classList.add('ring-2', 'ring-violet-500', 'ring-offset-2');
    } else {
        updateColorSelection(colorText.value);
    }
});
</script>
@endpush
