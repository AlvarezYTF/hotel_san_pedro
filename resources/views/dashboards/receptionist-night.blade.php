...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h3 class="font-bold text-gray-900">Estado del Turno: 
                        <span class="{{ $activeShift ? 'text-indigo-600' : ($pendingReception ? 'text-amber-600' : (($hasGlobalActiveShift || $hasGlobalPendingHandover) ? 'text-blue-600' : 'text-gray-500')) }}">
        ...
        @if($user->hasRole('Administrador') && $operationalShift)
            <div class="mt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3 rounded-lg border border-indigo-100 bg-indigo-50 p-3">
                ...
                <div class="flex items-center gap-2">
                    <button onclick="document.getElementById('modalForceClose').classList.remove('hidden')" class="text-xs font-bold bg-indigo-600 text-white px-3 py-2 rounded-lg hover:bg-indigo-700 transition-colors">
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-start gap-3">
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-2 mt-3">
                ...
                @endif
                @if($canStartShift)
                    <button onclick="document.getElementById('modalStartShift').classList.remove('hidden')" class="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-2 rounded-lg text-xs font-bold transition-colors">Iniciar turno</button>
                ...
                @endif
                @if($user->hasRole('Administrador') && $operationalShift)
                    <button onclick="document.getElementById('modalForceClose').classList.remove('hidden')" class="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-2 rounded-lg text-xs font-bold transition-colors">Forzar cierre operativo</button>
                @endif
            </div>
        </div>
    </div>
        ...
        <div class="mt-3">
            ...
            <form action="{{ route('shift.start') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="receptionist_name" value="{{ old('receptionist_name', auth()->user()->name ?? '') }}" class="w-full px-3 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500" maxlength="120" required>
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Base Inicial en Caja ($)</label>
                    <input type="text" name="base_inicial" value="{{ old('base_inicial') }}" oninput="formatNumberInput(this)" class="w-full px-3 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="0" required>
                </div>
        ...
        <div class="mt-3">
            ...
            <form action="{{ route('shift.force-close') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="reason" rows="3" class="w-full px-3 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ej: Turno atascado o sin recepción" required>Romper cadena de turnos desde panel</textarea>
                </div>
                <div class="flex gap-3">
