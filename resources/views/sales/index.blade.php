...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4 sm:p-6"
         x-data="{ 
             ...
             receptionists: @json($receptionists->mapWithKeys(function($receptionist) {
         ...
         ">
        <div class="space-y-6">
            <form id="filter-form" method="GET" action="{{ route('sales.index') }}">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="debt_status"
                                ...
                                <option value="pagado" {{ request('debt_status') == 'pagado' ? 'selected' : '' }}>Pagado</option>
                                <option value="pendiente" {{ request('debt_status') == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                            </select>
                        ...
                        <div class="relative">
                            <select name="receptionist_id"
                                ...
                                @foreach($receptionists as $receptionist)
                                    ...
                                    @endphp
                                    <option value="{{ $receptionist->id }}" {{ request('receptionist_id') == $receptionist->id ? 'selected' : '' }}>
                                        {{ $displayName }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="shift"
                                    ...
                                    @change="if (isShiftLocked) { shift = receptionists[receptionistId]; } else { document.getElementById('filter-form').submit(); }"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                    <div class="lg:col-span-3 space-y-2">
                        ...
                        <div class="flex items-center bg-gray-50 border border-gray-200 rounded-xl p-1">
                            <a href="{{ route('sales.index', array_merge(request()->except(['date']), ['date' => $currentDate->copy()->subMonth()->format('Y-m-d')])) }}" 
                            ...
                            <a href="{{ route('sales.index', array_merge(request()->except(['date']), ['date' => $currentDate->copy()->addMonth()->format('Y-m-d')])) }}" 
                               class="p-2 hover:bg-white hover:shadow-sm rounded-lg transition-all text-gray-400">
                        ...
                        <div class="overflow-x-auto pb-2 custom-scrollbar">
                            <div class="flex space-x-2 min-w-max">
                                @foreach($daysInMonth as $day)
                                    ...
                                    @endphp
                                    <a href="{{ route('sales.index', array_merge(request()->except(['date']), ['date' => $dayKey])) }}"
            ...
            <div class="pt-4 border-t border-gray-100">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="payment_method"
                                ...
                                <option value="ambos" {{ request('payment_method') == 'ambos' ? 'selected' : '' }}>Ambos</option>
                                <option value="pendiente" {{ request('payment_method') == 'pendiente' ? 'selected' : '' }}>Pendiente</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <i class="fas fa-chevron-down text-gray-400 text-xs"></i>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="room_id"
                                    ...
                                    class="block w-full pl-3 pr-10 py-2.5 border border-gray-300 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent appearance-none bg-white">
                                <option value="">Todas</option>
                                <option value="normal" {{ request('room_id') == 'normal' ? 'selected' : '' }}>Personas Corrientes</option>
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 pt-4 border-t border-gray-100 mt-4">
                    <a href="{{ route('sales.index', ['date' => $selectedDate]) }}"
                       class="inline-flex items-center justify-center px-4 py-2 rounded-lg border border-gray-200 bg-white text-gray-700 text-sm font-medium hover:bg-gray-50 transition-all duration-200">
                    ...
                    <button type="submit"
                            form="filter-form"
                            class="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-green-600 text-white text-sm font-medium hover:bg-green-700 transition-all duration-200 shadow-sm hover:shadow-md">
